...
def format_annotations(annotations_path): #, trainable_classes_path):
    ...
    with open(annotations_path) as annofile:
        for row in csv.reader(annofile):
                
            #if row[2] in trainable_classes:
                annotation = {'id': row[0], 'label': class_label(row[2]), 'confidence': row[3], 'x0': row[4],
...
def class_label(label_name):
    class_label = []
    if(label_name == '/m/0k4j'):
        class_label = 'Car'
    elif (label_name == '/m/07r04'):
        class_label = 'Truck'
    elif (label_name == '/m/01jfm_'):
        class_label = 'Licence plate'
    elif (label_name == '/m/04_sv'):
        class_label = 'Motorcycle'
    elif (label_name == '/m/015qff'):
